...
def demo(cfg):
    ...
    if cfg.DETECTION.ENABLE and cfg.DEMO.PREDS_BOXES != "":
        ...
        try: 
            n_s = frame_provider.n_seconds
            n_vt = frame_provider.n_video_time
            frames_logs = dict()

            for idx, sec in enumerate(n_s):
                frame_no = f'{"frame_"}{idx+1}'
                duration = sec
                timeline = str(n_vt[idx])
                frames_logs[frame_no] = {'duration': duration, 'timeline': timeline}
            
            try:
                json_object = json.dumps(frames_logs, indent=4)
                with open(cfg.DEMO.OUTPUT_FRAMES_FILE, "w") as outfile:
                    outfile.write(json_object)
            except Exception as e:
                print("Error in saving the JSON file of frames data: ", e)

        except Exception as e:
            print("Error while creating the frames result file: ", e)
